...
from processing import HandBuffer, compute_asymmetry, get_fft_arrays
...
WINDOW_SECONDS = 4.0  # rolling buffer length for FFT
...
def _draw_spectrum(
    ...
    """
    Render FFT magnitude vs frequency (0.5–15 Hz) in a small panel.
...
def _draw_amplitude_bar(
    ...
    """
    Draw a vertical bar whose fill height encodes tremor amplitude.
    """
...
def _render_hud(
    ...
    results_right: Dict[str, float],
    spec_left: tuple,      # (freqs_left, mags_left)
    spec_right: tuple,     # (freqs_right, mags_right)
    fps: float,
    frame_count: int
) -> None:
    H, W = frame.shape[:2]
    ...
    # Key legend
    _put_text(frame, "[Q] Quit  [R] Reset  [S] Snapshot",
              20, H - 15, _CLR["dim"], scale=0.38)


# ── Live loop ───────────────────────────────────────────────────────────────

def run_live(camera_index: int = 0, width: int = 1280, height: int = 720) -> None:
    ...
    fps_est = cap.get(cv2.CAP_PROP_FPS) or 30.0
    ...
    cv2.namedWindow("TremorQuant Live", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("TremorQuant Live", width, height)
    ...
    print("[INFO] Live tremor view running. Press Q or Esc to quit.")
    ...
    try:
        while True:
            ...
            cv2.imshow("TremorQuant Live", frame)

            key = cv2.waitKey(1) & 0xFF
            if key in (ord('q'), ord('Q'), 27):
                break
            elif key in (ord('r'), ord('R')):
                left_buf.y_series.clear()
                right_buf.y_series.clear()
                print("[INFO] Histories reset.")
            elif key in (ord('s'), ord('S')):
                fname = f"snapshot_live_{frame_count:06d}.png"
        ...
        hands_model.close()
        print("[INFO] Live UI stopped cleanly.")
